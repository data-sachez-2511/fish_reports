...
class MyWord2Vec():
    ...
    def fit(self):
        def list2gen():
            for i in range(self.db.length()):
                yield self.db.select(i)
        doc = list2gen()
        self.w2v_model.build_vocab(doc)
        self.w2v_model.train(doc, total_examples=self.w2v_model.corpus_count, epochs=50)
